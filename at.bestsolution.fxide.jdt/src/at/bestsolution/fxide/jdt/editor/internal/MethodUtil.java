 ...
 *******************************************************************************/
package at.bestsolution.fxide.jdt.editor.internal;
...
//	private final CompletionProposal proposal;
	...
	private final char[] methodSignature;
	private final boolean constructor;
	private final char[] declSignature;
	...
	public IMethod resolve() throws JavaModelException {
		String[] parameters = Signature.getParameterTypes(String.valueOf(SignatureUtil.fix83600( this.methodSignature )));
		for (int i= 0; i < parameters.length; i++) {
			parameters[i]= SignatureUtil.getLowerBound(parameters[i]);
		}

		return findMethod(String.valueOf(name), parameters, constructor, type);
	}
	...
	private IMethod findMethod(String name, String[] paramTypes, boolean isConstructor, IType type) throws JavaModelException {
		Map<String, char[]> typeVariables= computeTypeVariables(declSignature,type);
	 ...
	 * @throws JavaModelException if accessing the java model fails
	 */
	private static Map<String, char[]> computeTypeVariables(char[] declarationSignature, IType type) throws JavaModelException {
	...
	private IMethod findMethod(String name, String[] paramTypes, boolean isConstructor, IType type, Map<String, char[]> typeVariables) throws JavaModelException {
		System.err.println(name + "(" + Arrays.toString(paramTypes) + ")");
		IMethod[] methods = type.getMethods();
		for (int i= methods.length - 1; i >= 0; i--) {
			System.err.println("CHECKING: " + methods[i]);
